...
# =====================================================================
MODE_OFFLINE = True  # <--- UBAH KE False JIKA INGIN PAKAI KAMERA LIVE
STATION_MODE = 2  # 1 untuk Aksesoris, 2 untuk Case & Pianika
...
class DataManager:
    def __init__(self, station_id):
        ...
        if not os.path.exists(self.log_file):
            with open(self.log_file, mode='w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Waktu", "Station", "Model", "Status", "Komponen Kurang", "No_Tag", "NIK", "Nama_Operator", "Latency_ms", "Mode_Input"])
    ...
    def catat_log_csv(self, model, is_pass, missing_items, operator_data, latency_ms):
        ...
        input_mode = "Offline (Local File)" if MODE_OFFLINE else "Live Camera"
        
        try:
            with open(self.log_file, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([waktu, f"Station {self.station_id}", model, status_teks, kurang_teks, notag, nik, nama, f"{latency_ms:.2f}", input_mode])
        except Exception as e:
...
class MainController:
    def __init__(self, root):
        ...
        self.ui.btn_inspect.configure(command=self.mulai_inspeksi)
        self.ui.btn_save.configure(command=self.simpan_hasil)
        ...
        self.operator_db = []
        self.load_operator_db() 
        self.ui.entry_notag.bind("<Return>", self.proses_scan_notag) 
        self.ui.btn_ok_tag.configure(command=self.proses_scan_notag) 
        self.ui.entry_notag.focus() 
        ...
        if not MODE_OFFLINE:
            ...
            self.ui.lbl_cam_status.configure(text="● OFFLINE MODE", text_color=STATUS_COLORS["warning"])
            self.ui.update_status("Sistem Terkunci. Silakan Scan Tag Operator!", STATUS_COLORS["warning"])
            self.ui.camera_canvas.delete("all")
            self.ui.camera_canvas.create_text(
                self.ui.camera_canvas.winfo_reqwidth()//2, 
                self.ui.camera_canvas.winfo_reqheight()//2,
                text="[ OFFLINE MODE ]\nKlik 'Mulai Inspeksi' untuk memilih file gambar", 
    ...
    def proses_scan_notag(self, event=None):
        ...
        for op in self.operator_db:
            if op.get("notag") == scanned_tag:
                ...
                pesan = f"Login Sukses: {op.get('nama')}. "
                pesan += "Pilih file lokal untuk inspeksi." if MODE_OFFLINE else "Siap Inspeksi."
                self.ui.update_status(pesan, STATUS_COLORS["pass"])
                break
    ...
    def mulai_inspeksi(self):
        ...
        if MODE_OFFLINE:
            ...
            if not file_path:
                self.ui.update_status("Dibatalkan: Tidak ada file dipilih.", STATUS_COLORS["warning"])
                return
            ...
            if img_bgr is None:
                self.ui.update_status("Gagal: Format gambar rusak / tidak didukung!", STATUS_COLORS["fail"])
                return
    ...
    def reset_sistem(self):
        ...
        pesan_reset = "Sistem Direset. Pilih gambar untuk mulai." if MODE_OFFLINE else "Sistem Direset. Menunggu..."
        self.ui.update_status(pesan_reset, STATUS_COLORS["info"])
        
        for widget in self.ui.comp_scroll.winfo_children(): widget.destroy()
        self.ui.comp_count_label.configure(text="0/0")
        
        # Kembalikan tampilan kanvas khusus saat di offline mode
        if MODE_OFFLINE:
            self.ui.camera_canvas.delete("all")
            self.ui.camera_canvas.create_text(
                self.ui.camera_canvas.winfo_width()//2, 
                self.ui.camera_canvas.winfo_height()//2,
                text="[ OFFLINE MODE ]\nKlik 'Mulai Inspeksi' untuk memilih file gambar", 
                fill="white", font=("Segoe UI", 16, "bold"), justify="center"
            )
